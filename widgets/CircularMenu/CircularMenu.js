 ...
 * @extends TUIOWidget
 */
export class CircularMenu extends TUIOWidget {
  ...
  isTouched(x, y) {
    if (!this.isHide) {
      for (let i = 0; i < this.menuItemCoord.length; i += 1) {
        if (
          x >= this.menuItemCoord[i].xmin &&
          x <= this.menuItemCoord[i].xmax &&
          y >= this.menuItemCoord[i].ymin &&
          y <= this.menuItemCoord[i].ymax
        ) {
          return true;
        }
      }
  ...
  _onTouchCreation(tuioTouch) {
    if (this.isTouched(tuioTouch.x, tuioTouch.y)) {
      ...
      for (let i = 0; i < this.menuItemCoord.length; i += 1) {
        if (
          tuioTouch.x >= this.menuItemCoord[i].xmin &&
          tuioTouch.x <= this.menuItemCoord[i].xmax &&
          tuioTouch.y >= this.menuItemCoord[i].ymin &&
          tuioTouch.y <= this.menuItemCoord[i].ymax
        ) {
